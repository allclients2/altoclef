...
public class ShootArrowSimpleProjectileTask extends Task {
    ...
    private static Rotation calculateThrowLook(AltoClef mod, Entity target) {
        ...
        if (Float.isNaN(pitch)) {
            return new Rotation(target.getYaw(1), target.getPitch(1));
        } else {
    ...
    private static float Vec3dToYaw(AltoClef mod, Vec3d vec) {
        return (mod.getPlayer().getYaw(1) +
                MathHelper.wrapDegrees((float) Math.toDegrees(Math.atan2(vec.getZ() - mod.getPlayer().getZ(), vec.getX() - mod.getPlayer().getX())) - 90f - mod.getPlayer().getYaw(1)));
    }
